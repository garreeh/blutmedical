<?php

// Define table and primary key
$table = 'product_image';
$primaryKey = 'product_image_id';

// Define columns for DataTables
$columns = array(
  array(
    'db' => 'product_image_id',
    'dt' => 0,
    'field' => 'product_image_id',
    'formatter' => function ($lab1, $row) {
      return $row['product_image_id'];
    }
  ),

  array(
    'db' => 'product_image_path',
    'dt' => 1,
    'field' => 'product_image_path',
    'formatter' => function ($lab3, $row) {
      // Return an HTML <img> tag with the image path
      // $imageUrl = '../../uploads/' . basename($row['product_image']);
      // return '<img src="' . $imageUrl . '" alt="Product Image" style="max-width: 100px; height: auto;">';
      return '<a class="fetchDataProductImage" href="#"> View Image</a> ';
    }
  ),

  array(
    'db' => 'product_image_id',
    'dt' => 2,
    'field' => 'product_image_id',
    'formatter' => function ($lab5, $row) {
      return '
            <div class="dropdown">
                <button class="btn btn-info" type="button" id="dropdownMenuButton' . $row['product_image_id'] . '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    &#x22EE;
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton' . $row['product_image_id'] . '">
                    <a class="dropdown-item fetchDataProductImagesEdit" href="#">Edit</a>

                </div>
            </div>';
    }
  ),
);

// Database connection details
include '../../connections/ssp_connection.php';

// Include the SSP class
require('../../assets/datatables/ssp.class_with_where.php');

$product_id = isset($_GET['product_id']) ? intval($_GET['product_id']) : 0;

$where = "product_id = '$product_id'";

// Fetch and encode ONLY WHERE
echo json_encode(SSP::simple($_GET, $sql_details, $table, $primaryKey, $columns, $where));
