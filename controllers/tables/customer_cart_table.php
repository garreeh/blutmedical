...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  $groupKey = !empty($row['user_id'])
    ? $row['user_id']
    : 'GUEST_' . md5($row['delivery_guest_fullname']);
...
foreach ($grouped as $row) {
  ...
  if ($status === 'Cart') {
    $status = "<span style='background:#e2e3e5;color:#383d41;padding:4px 8px;border-radius:6px;font-weight:600;'>$status</span>";
  ...
  $dropdown = '
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                <a class="dropdown-item fetchDataRemind"
                    data-id="' . $row['cart_id'] . '"
                    href="#">
                    Remind Customer
                </a>
