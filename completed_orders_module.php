            ...
            <table class="table">
              <thead>
                <tr>
                  ...
                  <th class="product-date">Date Delivered</th>
          ...
          if (response.success) {
            if (response.items.length > 0) {
              $.each(response.items, function (index, item) {
                ...
                cartContent += '</div>';
                cartContent += '</td>';
                cartContent += '<td style="background-color: lightgreen;"> <strong>' + item.cart_status + '<strong></td>'; // Display total price with variation if available

                cartContent += '<td>₱ ' + (variationPrice * cartQuantity).toFixed(2) + '</td>'; // Display total price with variation if available
                cartContent += '<td <strong>' + formattedDateTime + '<strong></td>'; // Display total price with variation if available
